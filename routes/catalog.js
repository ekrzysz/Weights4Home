...
router.get('/checkout', function(req, res, next) {
    ...
    let insertquery = "INSERT INTO orderdetail(orderdetail_id, product_id, saleprice, qty) VALUES (?, ?, (SELECT price from weight_products where product_id = " + cartitem + "), ?)";
...
let query = "SELECT product_id, product_name, brand, pound, color, des, price FROM weight_products";
// execute query
...
router.get('/cart', function(req, res, next) {
    ...
    let query = "SELECT product_id, product_name, brand, pound, color, des, price FROM weight_products WHERE product_id IN (" + req.session.cart + ") order by find_in_set(product_id, '" + req.session.cart + "');";
    // execute query
    db.query(query, (err, result) => {
    if (err) {res.render('error');} else
    {res.render('cart', {cartitems: result, qtys:
    req.session.qty });}
    });
    }
    });
